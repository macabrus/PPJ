...
public class LRParser {
	...
	public LRParser(ArrayList<LexUnit> input,
			HashMap<Integer, HashMap<String, Action>> actions,
			HashMap<Integer, HashMap<String, Integer>> newState,
			HashSet<String> syncro) {
		super();
	...
	private void parse() {
		...
		while (true) {
			...
			LexUnit lu;
			Integer state = states.firstElement();
			Action action;
			...
			if (action.isReduce()) {
				...
				if (prod.right.size() == 0) {
					...
					for (int i = 0; i < prod.right.size(); ++i) {
						states.pop();
						newNode.addChild(characters.firstElement());
						characters.pop();
					}
				}
				...
				Integer state1 = states.firstElement();
